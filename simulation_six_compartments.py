...
def sy_prime(y_susceptible, infected, y_recovered, young, population):
    return -beta * y_susceptible * infected / population + theta * y_recovered + \
           birth_rate * young - death_rate_young(young / population) * y_susceptible - \
           aging_rate * y_susceptible
...
def iy_prime(y_susceptible, infected, y_infected, young, population):
    return beta * y_susceptible * infected / population - gamma * y_infected - \
           death_rate_young(young / population) * y_infected - \
...
def ry_prime(y_infected, y_recovered, young, population):
    return gamma * y_infected - theta * y_recovered - \
           death_rate_young(young / population) * y_recovered - aging_rate * y_recovered
...
def r_prime(infected, y_recovered, o_recovered, young, population):
    return gamma * infected - theta * (y_recovered + o_recovered) - \
           death_rate_young(young / population) * y_recovered - \
...
def the_magic(filename):
    ...
    ry_data = solution[4]
    ro_data = solution[5]
    s_data = [a + b for a, b in zip(sy_data, so_data)]
    i_data = [a + b for a, b in zip(iy_data, io_data)]
    r_data = [a + b for a, b in zip(ry_data, ro_data)]
    n_data = solution[6]
    y_data = solution[7]
    o_data = solution[8]
    ...
    initial_GR = n_prime(iy_data[0], io_data[0], y_data[0], o_data[0], n_data[0]) / n_data[0]
    ...
    if final_GR >= initial_GR:
        ...
        if (death_rate_young(y_data[0] / n_data[0])) < death_rate_young(y_data[-1] / n_data[-1]):
