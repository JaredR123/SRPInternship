...
young_to_old_step = float(input_lines[13])
iteration_max = int(input_lines[14])
iteration_step = float(input_lines[15])
population_step = 0
iteration = 0
...
def death_rate_young(young_to_old):
    return 1 / (5 + (numpy.e ** (20 * (1 / young_to_old - 0.18)))) + 0.15
...
def age_ratio_solver(young_to_old):
    return (birth_rate + death_rate_old - death_rate_young(young_to_old)) / \
           (1 + young_to_old) - aging_rate


def the_magic(filename):
    # Solves for the young to old ratio before the pathogen
    young_to_old = root_scalar(age_ratio_solver, method="secant", x0=0.5, x1=2).root
    print(young_to_old)
